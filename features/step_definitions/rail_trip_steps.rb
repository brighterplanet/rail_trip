...
Given /^the geocoder will encode the (.*) as "([^\"]*)" in "([^\"]*)"$/ do |location, lat_lng, country|
  if lat_lng.present?
    result = mock Geocoder::Result, :coordinates => lat_lng.split(',').map(&:to_f), :country_code => country
  else
    result = nil
  end
